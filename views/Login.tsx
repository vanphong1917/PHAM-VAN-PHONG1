...
import { Mail, Lock, Shield, Layers, ArrowRight, AlertCircle, ChevronLeft, CheckCircle2 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/60 border border-slate-200 overflow-hidden">
          {/* Top Branding / Success Header */}
          <div className="p-8 pb-6 text-center">
            {view === 'RESET_SUCCESS' ? (
              <div className="inline-flex items-center justify-center p-3 bg-emerald-100 rounded-2xl text-emerald-600 mb-6 shadow-lg shadow-emerald-50">
            ...
            <h1 className="text-2xl font-bold text-slate-900 mb-2">
              {view === 'FORGOT_PASSWORD' ? t('forgotPasswordTitle') : 
               view === 'RESET_SUCCESS' ? t('sendResetLink') : t('loginTitle')}
            </h1>
            <p className="text-slate-500 text-sm">
              {view === 'FORGOT_PASSWORD' ? t('forgotPasswordDesc') : 
               view === 'RESET_SUCCESS' ? t('resetLinkSent') : t('hotStorageDesc')}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 pt-0 space-y-5">
            {view === 'RESET_SUCCESS' ? (
              <div className="space-y-4">
                <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl text-emerald-800 text-sm leading-relaxed text-center font-medium">
                  {t('resetLinkSent')}
                </div>
                <button 
                  type="button"
                  onClick={() => setView('LOGIN')}
                  className="w-full flex items-center justify-center gap-2 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition-all"
                >
                  <ChevronLeft size={18} />
                  {t('backToLogin')}
                </button>
              ...
              <>
                ...
                <div className="flex items-center justify-between py-1">
                  {view === 'LOGIN' ? (
                    ...
                    <button 
                      type="button" 
                      onClick={() => setView('LOGIN')}
                      className="flex items-center gap-1 text-xs text-slate-500 font-bold hover:text-indigo-600 transition-colors"
                    >
                      <ChevronLeft size={14} />
                      {t('backToLogin')}
                    </button>
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      {view === 'LOGIN' ? t('login') : t('sendResetLink')}
